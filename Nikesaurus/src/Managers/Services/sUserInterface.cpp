...
namespace NIKE {
	...
	bool UI::Service::sliderHovered(Entity::Type entity) const {
		...
		// Assume trans.position is the center; half the size is half of trans.scale.
		Vector2f halfSize = trans.scale * 0.5f;
		// Use the mouse window position
	...
	void UI::Service::updateSliders() {
		// Process BGMSlider (Music Volume Slider)
		{
			...
			/*
			if (bgmSliderOpt.has_value())
				cout << "Have slider" << endl;

			if (bgmSliderBarOpt.has_value())
				cout << "Have bar slider" << endl;
			*/

			if (bgmSliderOpt.has_value() && bgmSliderBarOpt.has_value()) {
				Entity::Type slider = bgmSliderOpt.value();
				Entity::Type bar = bgmSliderBarOpt.value();
				auto sliderTransOpt = NIKE_ECS_MANAGER->getEntityComponent<Transform::Transform>(slider);
				//cout << slider.v
				auto barTransOpt = NIKE_ECS_MANAGER->getEntityComponent<Transform::Transform>(bar);
				if (sliderTransOpt.has_value() && barTransOpt.has_value()) {
					...
					// Compute the effective left and right bounds by offsetting the bar's edges by half the slider knob's width.
					float effectiveLeft = barTrans.position.x - (barTrans.scale.x * 0.5f) + (sliderTrans.scale.x * 0.5f);
					float effectiveRight = barTrans.position.x + (barTrans.scale.x * 0.5f) - (sliderTrans.scale.x * 0.5f);

					// If the slider knob is hovered and the left mouse button is pressed, update its x–position.
					if (sliderHovered(slider) && NIKE_INPUT_SERVICE->isMousePressed(NIKE_MOUSE_BUTTON_LEFT)) {
						Vector2f mousePos = NIKE_INPUT_SERVICE->getMouseWindowPos();
						sliderTrans.position.x = std::clamp(mousePos.x, effectiveLeft, effectiveRight);
					}

					// Compute relative value (0 at effectiveLeft, 1 at effectiveRight) for volume setting.
					float relValue = (sliderTrans.position.x - effectiveLeft) / (effectiveRight - effectiveLeft);
					bgmVolume = relValue;
		...
		// Process SFXSlider (SFX Volume Slider)
		{
			...
			if (sfxSliderOpt.has_value() && sfxSliderBarOpt.has_value()) {
				Entity::Type slider = sfxSliderOpt.value();
				Entity::Type bar = sfxSliderBarOpt.value();
				auto sliderTransOpt = NIKE_ECS_MANAGER->getEntityComponent<Transform::Transform>(slider);
				auto barTransOpt = NIKE_ECS_MANAGER->getEntityComponent<Transform::Transform>(bar);
				if (sliderTransOpt.has_value() && barTransOpt.has_value()) {
					auto& sliderTrans = sliderTransOpt.value().get();
