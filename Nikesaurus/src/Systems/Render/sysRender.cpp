...
namespace NIKESAURUS {
	...
	void Render::Manager::renderObject(Matrix_33 const& x_form, Render::Shape const& e_shape) {
		...
		//Get model
		auto model = NIKE_ENGINE.getService<Assets::Service>()->getModel(e_shape.model_ref);
	...
	void Render::Manager::renderObject(Matrix_33 const& x_form, Render::Texture const& e_texture) {
		...
		glBindTextureUnit(
			...
			NIKE_ENGINE.getService<Assets::Service>()->getTexture(e_texture.texture_ref)->gl_data
		);

		glTextureParameteri(NIKE_ENGINE.getService<Assets::Service>()->getTexture(e_texture.texture_ref)->gl_data, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTextureParameteri(NIKE_ENGINE.getService<Assets::Service>()->getTexture(e_texture.texture_ref)->gl_data, GL_TEXTURE_WRAP_T, GL_REPEAT);
		...
		//Get model
		auto model = NIKE_ENGINE.getService<Assets::Service>()->getModel("square-texture");
	...
	void Render::Manager::renderText(Render::Text const& e_text) {
		...
		//Get model
		const auto& model = NIKE_ENGINE.getService<Assets::Service>()->getModel("square-texture");
		...
		for (char c : e_text.text) {
			const Assets::Font::Character& ch = NIKE_ENGINE.getService<Assets::Service>()->getFont(e_text.font_ref)->char_map.at(c);
	...
	void Render::Manager::renderWireFrame(Matrix_33 const& x_form, Render::Color const& e_color) {
		...
		//Get model
		auto model = NIKE_ENGINE.getService<Assets::Service>()->getModel("square");
	...
	void Render::Manager::transformAndRenderEntity(Entity::Type entity, bool debugMode) {
		...
		//Check If Shape
		if (NIKE_ENGINE.getService<Coordinator::Manager>()->checkEntityComponent<Render::Shape>(entity)) {
			auto& e_shape = NIKE_ENGINE.getService<Coordinator::Manager>()->getEntityComponent<Render::Shape>(entity);
		...
		else if (NIKE_ENGINE.getService<Coordinator::Manager>()->checkEntityComponent<Render::Texture>(entity)) {
			auto& e_texture = NIKE_ENGINE.getService<Coordinator::Manager>()->getEntityComponent<Render::Texture>(entity);

			//Copy transform for texture mapping
			Transform::Transform copy = e_transform;
			Vector2f tex_size{ (float)NIKE_ENGINE.getService<Assets::Service>()->getTexture(e_texture.texture_ref)->size.x, (float)NIKE_ENGINE.getService<Assets::Service>()->getTexture(e_texture.texture_ref)->size.y };
		...
		if (debugMode) {
			...
			if (NIKE_ENGINE.getService<Coordinator::Manager>()->checkEntityComponent<Transform::Velocity>(entity)) {
				auto const& e_velo = NIKE_ENGINE.getService<Coordinator::Manager>()->getEntityComponent<Transform::Velocity>(entity);
	...
	void Render::Manager::update() {
		...
		//	//if (NIKE_ENGINE.getService<Coordinator::Manager>()->checkEntityComponent<Render::Text>(entity)) {
		//	//	renderText(NIKE_ENGINE.getService<Coordinator::Manager>()->getEntityComponent<Render::Text>(entity));
		//	//	continue;
		//	//}
		//	//Transform and render object
		//	transformAndRenderEntity(entity, true);
