...
namespace NIKESAURUS {
	...
	void Shader::Manager::useShader(const std::string& shader_ref) {
		glUseProgram(NIKE_ENGINE.getService<Assets::Service>()->getShader(shader_ref));
	}
	...
	void Shader::Manager::setUniform(const std::string& shader_ref, const std::string& name, int value) {
		int location = glGetUniformLocation(NIKE_ENGINE.getService<Assets::Service>()->getShader(shader_ref), name.c_str());
		if (location >= 0) {
