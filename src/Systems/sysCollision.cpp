...
namespace Collision {
    ...
    void Manager::update() {
        for (size_t i = 0; i < colliders.size(); ++i) {
            ...
            for (size_t j = i + 1; j < colliders.size(); ++j) {
                if (!colliders[j].active) continue;
    ...
    void Manager::registerCollider(const Vector2& position, const Vector2& size, float radius, const Vector2& velocity) {
        for (auto& collider : colliders) {
            if (!collider.active) {
    ...
    void Manager::unregisterCollider(const Vector2& position) {
        for (auto& collider : colliders) {
    ...
    bool Manager::detectCollision(const Collider& a, const Collider& b) const {
